...
static ssize_t _well_known_core_handler(coap_pkt_t* pdu, uint8_t *buf, size_t len, void *p);
                                                         ...
                                                         sock_udp_ep_t *remote)
{
    ...
    ssize_t pdu_len = resource->handler(pdu, buf, len, resource->parameter);
    if (pdu_len < 0) {
